...
async def insert_bizcontacts(bizinfo_data: BizContactsDataRequests, db: AsyncSession = Depends(get_db), current_user: str = Depends(get_current_user)):
    try:
...
async def search_bizcontacts(order_id: int, db: AsyncSession = Depends(get_db), current_user: str = Depends(get_current_user)): # 인증 종속성 추가
    try:
...
async def delete_bizcontacts(order_id:int, db: AsyncSession = Depends(get_db), current_user: str = Depends(get_current_user)):
    try:
